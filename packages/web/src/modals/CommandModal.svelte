...
<FormProviderCore {values}>
  <ModalBase {...$$restProps}>
    <svelte:fragment slot="header">{_t('commandModal.configureCommand', { defaultMessage: 'Configure command' })}</svelte:fragment>
    ...
    <svelte:fragment slot="footer">
      <FormSubmit
        value={_t('commandModal.ok', { defaultMessage: 'OK' })}
        on:click={e => {
          closeCurrentModal();
      ...
      <FormStyledButton
        type="button"
        value={_t('commandModal.reset', { defaultMessage: 'Reset' })}
        on:click={() => {
          closeCurrentModal();
          apiCall('config/update-settings', {
      ...
      />
      <FormStyledButton type="button" value={_t('commandModal.close', { defaultMessage: 'Close' })} on:click={closeCurrentModal} />
    </svelte:fragment>
  </ModalBase>
</FormProviderCore>
