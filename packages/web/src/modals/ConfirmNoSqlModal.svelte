...
<FormProvider>
  <ModalBase {...$$restProps}>
    <div slot="header">{_t('confirmNoSqlModal.saveChanges', { defaultMessage: 'Save changes' })}</div>
    ...
    {#if skipConfirmSettingKey}
      <div class="mt-2">
        <TemplatedCheckboxField
          label={_t('confirmNoSqlModal.dontAskAgain', { defaultMessage: "Do not ask again" })}
    ...
    <div slot="footer">
      ...
      <FormStyledButton
        type="button"
        value={_t('confirmNoSqlModal.openScript', { defaultMessage: 'Open script' })}
        on:click={() => {
          newQuery({
            initialData: script,
          });
