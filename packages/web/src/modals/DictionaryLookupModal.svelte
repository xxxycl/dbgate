...
<FormProvider>
  <ModalBase {...$$restProps}>
    <svelte:fragment slot="header">{_t('dictionaryLookupModal.lookupFrom', { defaultMessage: 'Lookup from {pureName}', values: { pureName } })}</svelte:fragment>

    <!-- <FormTextField name="search" label='Search' placeholder="Search" bind:value={search} /> -->
    <div class="largeFormMarker">
      <SearchInput placeholder={_t('dictionaryLookupModal.searchPlaceholder', { defaultMessage: 'Search' })} bind:value={search} isDebounced />
    </div>
    ...
    <svelte:fragment slot="footer">
      {#if multiselect}
        <FormSubmit
          value={_t('dictionaryLookupModal.ok', { defaultMessage: 'OK' })}
          on:click={() => {
            closeCurrentModal();
        ...
        />
      {/if}
      <FormStyledButton type="button" value={_t('dictionaryLookupModal.close', { defaultMessage: 'Close' })} on:click={closeCurrentModal} />
