...
<FormProvider>
  <ModalBase {...$$restProps}>
    ...
    <svelte:fragment slot="footer">
      {#if isRunning}
        <FormStyledButton value={_t('runScriptModal.stop', { defaultMessage: 'Stop' })} on:click={handleStop} data-testid="RunScriptModal_stop" />
      {:else}
      ...
      {#if onOpenResult && !isRunning}
        <FormStyledButton
          value={openResultLabel || _t('runScriptModal.openResult', { defaultMessage: 'Open result' })}
          on:click={() => {
            closeCurrentModal();
            onOpenResult();
          }}
