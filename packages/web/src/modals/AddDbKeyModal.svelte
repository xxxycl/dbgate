...
<FormProvider>
  <ModalBase {...$$restProps}>
    <svelte:fragment slot="header">{_t('addDbKey.title', { defaultMessage: 'Add key' })}</svelte:fragment>

    <div class="container">
      <FormFieldTemplateLarge label={_t('addDbKey.keyName', { defaultMessage: 'Key' })} type="text" noMargin>
        <TextField
          value={keyName}
          on:change={e => {
            // @ts-ignore
      ...
      <FormFieldTemplateLarge label={_t('addDbKey.type', { defaultMessage: 'Type' })} type="combo" noMargin>
        <SelectField
