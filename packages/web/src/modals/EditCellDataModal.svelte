<script lang="ts">
  ...
  function handleFormatJson() {
    ...
    if (parsed) {
      ...
      showModal(ErrorMessageModal, { message: _t('editCellDataModal.notValidJson', { defaultMessage: 'Not valid JSON' }) });
    }
  }
</script>
...
<FormProvider>
  <ModalBase {...$$restProps}>
    <div slot="header">{_t('editCellDataModal.editCellValue', { defaultMessage: 'Edit cell value' })}</div>
    ...
    <div slot="footer" class="footer">
      ...
      <div>
        <FormStyledButton type="button" value={_t('editCellDataModal.formatJson', { defaultMessage: 'Format JSON' })} on:click={handleFormatJson} />

        {_t('editCellDataModal.codeHighlighting', { defaultMessage: 'Code highlighting' })}:
        <SelectField
          isNative
          value={syntaxMode}
          on:change={e => (syntaxMode = e.detail)}
          options={[
            { value: 'text', label: _t('editCellDataModal.noneRawText', { defaultMessage: 'None (raw text)' }) },
