...
{#if electron}
  <InlineButton on:click={handleOpenElectronFile} title={_t('inlineUploadButton.openFile', { defaultMessage: 'Open file' })} data-testid={$$props['data-testid']}>
    <FontIcon {icon} />
  </InlineButton>
{:else}
  <InlineButtonLabel on:click={() => {}} title={_t('inlineUploadButton.uploadFile', { defaultMessage: 'Upload file' })} data-testid={$$props['data-testid']} htmlFor={inputId}>
    <FontIcon {icon} />
  </InlineButtonLabel>
{/if}
