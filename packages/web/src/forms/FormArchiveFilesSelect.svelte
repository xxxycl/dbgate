...
<div class="wrapper">
  ...
  <div>
    <FormStyledButton
      type="button"
      value={_t('formArchiveFilesSelect.allFiles', { defaultMessage: 'All files' })}
      on:click={() => setFieldValue(name, _.uniq([...($values[name] || []), ...($files && $files.map(x => x.name))]))}
    />
    <FormStyledButton type="button" value={_t('formArchiveFilesSelect.removeAll', { defaultMessage: 'Remove all' })} on:click={() => setFieldValue(name, [])} />
  </div>
</div>
